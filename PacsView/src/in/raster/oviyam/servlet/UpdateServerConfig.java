...
/**
 * Updates the oviyam-config.xml file.
 * @author bharathi
 * @version 0.7
 *
 */
public class UpdateServerConfig extends HttpServlet{
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 567640663641233565L;
	
	/**
	 * Initalize the Logger.
	 */	
	private static Logger log = Logger.getLogger(UpdateServerConfig.class);
	...
	public void doPost(HttpServletRequest request,HttpServletResponse response)throws ServletException,IOException{
		...
		try {
			...
			/*
			 * Calls the updateElementValues(aeTitle, hostName, port,
			 * wadoPort,dcmProtocol). It will update the existing element values with
			 * with the given values.
			 */
			sxc.updateElementValues(aeTitle, hostName, port, wadoPort,dcmProtocol);
		} catch (Exception e) {
