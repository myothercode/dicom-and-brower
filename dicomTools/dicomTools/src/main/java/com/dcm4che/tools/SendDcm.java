 ...
 */
public class SendDcm extends StorageCommitmentService {
    ...
    private static final String USAGE =
            "SendDcm <aet>[@<host>[:<port>]] <file>|<directory>... [Options]";
    ...
    private static final String EXAMPLE =
            "\nExample: SendDcm STORESCP@localhost:11112 image.dcm -stgcmt -L SendDcm:11113 \n"
    ...
    private static CommandLine parse(String[] args) {
        ...
        OptionBuilder.withDescription(
                "set device name, use SendDcm by default");
        ...
        try {
            ...
            exit("SendDcm: " + e.getMessage());
            throw new RuntimeException("unreachable");
        }
        if (cl.hasOption('V')) {
            Package p = SendDcm.class.getPackage();
            System.out.println("SendDcm v" + p.getImplementationVersion());
            System.exit(0);
        }
    ...
    public static void sendAction(String AE,String filePath) {
        //String[] args=new String[]{"DCM4CHEE@localhost:11112","E:/MR/1.2.840.113619.2.5.1762583153.215519.978957063.177/1.2.840.113619.2.5.1762583153.215519.978957063.183.dcm"};
        String[] args=new String[]{AE,filePath,"-fileref"};
        CommandLine cl = parse(args);
        SendDcm SendDcm = new SendDcm(cl.hasOption("device")
                ? cl.getOptionValue("device") : "SendDcm");
        final List<String> argList = cl.getArgList();
        String remoteAE = argList.get(0);


        String[] calledAETAddress = split(remoteAE, '@');
        SendDcm.setCalledAET(calledAETAddress[0]);
        if (calledAETAddress[1] == null) {
            SendDcm.setRemoteHost("127.0.0.1");
            SendDcm.setRemotePort(104);
        } else {
            String[] hostPort = split(calledAETAddress[1], ':');
            SendDcm.setRemoteHost(hostPort[0]);
            SendDcm.setRemotePort(toPort(hostPort[1]));
        }
        if (cl.hasOption("L")) {
            String localAE = cl.getOptionValue("L");
            String[] localPort = split(localAE, ':');
            if (localPort[1] != null) {
                SendDcm.setLocalPort(toPort(localPort[1]));
            }
            String[] callingAETHost = split(localPort[0], '@');
            SendDcm.setCalling(callingAETHost[0]);
            if (callingAETHost[1] != null) {
                SendDcm.setLocalHost(callingAETHost[1]);
            }
        }
        SendDcm.setOfferDefaultTransferSyntaxInSeparatePresentationContext(
                cl.hasOption("ts1"));
        SendDcm.setSendFileRef(cl.hasOption("fileref"));
        if (cl.hasOption("username")) {
            ...
            SendDcm.setUserIdentity(userId);
        }
        SendDcm.setStorageCommitment(cl.hasOption("stgcmt"));
        ...
        if (cl.hasOption("stgcmtae")) {
            try {
                ...
                SendDcm.setStgcmtCalledAET(aet_hostport[0]);
                SendDcm.setRemoteStgcmtHost(host_port[0]);
                SendDcm.setRemoteStgcmtPort(toPort(host_port[1]));
            } catch (Exception e) {
        ...
        while( SendDcm.getLastSentFile() < SendDcm.getNumberOfFilesToSend() ) {
            try {
                SendDcm.start();
            } catch (Exception e) {
            ...
            try {
                ...
                SendDcm.send();
                t2 = System.currentTimeMillis();
                prompt(SendDcm, (t2 - t1) / 1000F);
                if (SendDcm.isStorageCommitment()) {
                    t1 = System.currentTimeMillis();
                    if (SendDcm.commit()) {
                        t2 = System.currentTimeMillis();
                        ...
                        try {
                            DicomObject cmtrslt = SendDcm.waitForStgCmtResult();
                            t1 = System.currentTimeMillis();
    ...
    protected static void prompt(SendDcm SendDcm, float seconds) {
        System.out.print("\nSent ");
        System.out.print(SendDcm.getNumberOfFilesSent());
        System.out.print(" objects (=");
        promptBytes(SendDcm.getTotalSizeSent());
        System.out.print(") in ");
        System.out.print(seconds);
        System.out.print("s (=");
        promptBytes(SendDcm.getTotalSizeSent() / seconds);
    ...
    private static void exit(String msg) {
        ...
        System.err.println("Try 'SendDcm -h' for more information.");
        System.exit(1);
    }
    ...
    public void addFile(File f) {
        ...
        try {
            ...
            System.out.print('F');
            map.put(info.f.getName(),e.getMessage());
            return;
        } finally {
    ...
    public void send() {
        ...
        for ( ; (i+lastSentFile) < n && (batchSize==0 || i < batchSize); ++i) {
            ...
            if (tc == null) {
                System.out.println();
                String infom=UIDDictionary.getDictionary().prompt(info.cuid) + " not supported by " + remoteAE.getAETitle();
                System.out.println(infom);
                System.out.println("skip file " + info.f);
                map.put(info.f.getName(),infom);
                continue;
            }
            ...
            if (tsuid == null) {
                System.out.println();
                String infom=UIDDictionary.getDictionary().prompt(
                        info.cuid)
                        + " with "
        ...
        switch (status) {
            ...
            case 0xB007:
                ...
                promptErrRSP("WARNING: Received RSP with Status ", status, info,
                        cmd);
                System.out.print('W');
                map.put(info.f.getName(),"WARNING: Received RSP with Status "+StringUtils.shortToHex(status));
                break;
            default:
                promptErrRSP("ERROR: Received RSP with Status ", status, info, cmd);
                System.out.print('F');
                map.put(info.f.getName(),"ERROR: Received RSP with Status "+StringUtils.shortToHex(status));
        }
    }
    ...
    private static InputStream openFileOrURL(String url) throws IOException {
        if (url.startsWith("resource:")) {
            return SendDcm.class.getClassLoader().getResourceAsStream(
                    url.substring(9));
        }
        try {
